...
# Función de kernel para suma de matrices
@cuda.jit
def matrix_add_kernel(A, B, C):
    """
    Kernel para la suma de matrices (C = A + B).
    Demuestra el paralelismo explícito para una operación de mapeo trivial.
    """
    i, j = cuda.grid(2)
    N, M = C.shape[0], C.shape[1]
...
def run_matrix_mult_gpu(A, B, threads_per_block=(32, 32)):
    ...
    tiempo_transfer = tiempo_transfer_htod + tiempo_transfer_dtoh
    tiempo_gpu_total = tiempo_transfer + tiempo_gpu_kernel
    
    return C_host, tiempo_gpu_total, tiempo_gpu_kernel, tiempo_transfer

# Función para ejecutar la suma de matrices en GPU
def run_matrix_add_gpu(A, B, threads_per_block=(32, 32)):
    """
    Ejecuta la suma de matrices en GPU, midiendo las fases explícitamente.
    """
    ...
    # Ejecución del kernel de suma (JIT)
    matrix_add_kernel[blocks_per_grid, threads_per_block](A_device, B_device, C_device)
    cuda.synchronize()
    tiempo_gpu_kernel = time.time() - start_kernel
